...
class AllDataUpdateFormType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        $formModifier = function (FormInterface $form, ?Region $region = null, ?Pays $selectedPays = null) {
            $paysChoices = $region !== null && $region->getLibelle() !== null
        ...
        $builder->addEventListener(FormEvents::PRE_SET_DATA, function (FormEvent $event) use ($formModifier, $regionFieldOptions) {
            ...
            if (!$incident instanceof AllData) {
                return;
            }

            $pays = $incident->getPays();
            if ($pays === null) {
                return;
            }

            $rawRegion = $pays->getRegion();
            if ($rawRegion === null) {
                return;
            }

            $region = $this->regionRepository->findCanonicalByLibelle($rawRegion->getLibelle()) ?? $rawRegion;
            $canonicalPays = $this->paysRepository->findCanonicalByLibelleForRegion(
                $pays->getLibelle(),
                $region->getLibelle()
            ) ?? $pays;

            $form = $event->getForm();
            $form->add('regions', EntityType::class, array_merge($regionFieldOptions, [
                'data' => $region,
            ]));
            $formModifier($form, $region, $canonicalPays);
        }, -10);
